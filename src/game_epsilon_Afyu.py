...
tm=monotonic
fr=fill_rect
DBT='gg00000000000ggg0ccc0ccc0ccc0g0ccc0ccccc0ccc00ccc0ccccc0ccc00ccc0ccccc0ccc0g0ccc0ccc0ccc0ggg000'
c=((26,28,44),(93,39,93),(177,62,83),(239,125,87),(255,205,117),(167,240,112),(56,183,100),(37,113,121),(41,54,111),(59,93,201),(65,166,246),(115,239,247),(244,244,244),(148,176,194),(86,108,134),(51,60,87))
...
'pdown':(3,0,15,20,
DBT+'000cc000gggg0ccccccccc0gggg00000000000ggggg04c848c40gggggg044444440ggggggg0000000ggggggg0cccdccc0ggggg0cccccdccc0gggg0c0cfcdc0c0gggg0c0cccdc0c0gggg000cfcdc000gggggg0cccdc0gggggggg0000000ggggggggg00g00ggggg'),
'pleft':(3,0,15,20,
DBT+'000cc000gggg0ccccccccc0gggg00000000000ggggg08c48c440gggggg044444440ggggggg0000000ggggggg0ccdcccc0ggggg0ccccdcccc0gggg0c0cfcd0cc0gggg0c0cccdc0c0gggg000cfcdc000gggggg0cccdc0gggggggg0000000ggggggg000gg000gggg'),
'pright':(2,0,15,20,
DBT+'cc000000gggg0ccccccccc0gggg00000000000ggggg044c84c80gggggg044444440ggggggg0000000ggggggg0ccccdcc0ggggg0ccccdcccc0gggg0cc0dcfc0c0gggg0c0cdccc0c0gggg000cdcfc000gggggg0cdccc0gggggggg0000000gggggggg000gg000ggg'),
'pup':(3,0,15,20,
DBT+'000cc000gggg0ccccccccc0gggg00000000000ggggg044444440gggggg044444440ggggggg0000000ggggggg0ccccccc0ggggg0ccccccccc0gggg0c0ccccc0c0gggg0c0ccccc0c0gggg000ccccc000gggggg0ccccc0gggggggg0000000ggggggggg00g00ggggg'),
'poele':(0,0,19,19,
...
def dt(t,x,y,c,b,f=None):draw_string(t,x,y,c,b)
...
def ds(sprite_name,x,y,mu=1):
  ...
  for i in range(h):
   for k in range(w):
    ...
    if px not in (16,17):fr(x+offx*mu+curx*mu,y+offy*mu+cury*mu,mu,mu,c[px])
    curx +=1
   curx=0
   cury+=1
...
class M:
 def __init__(self):
  self.mi=["p","s_cuit"]
  self.m_time=(tm(),randint(50,70))
  if getrandbits(1):self.mi.append("sa_cuite")
  if getrandbits(1):self.mi.append("o_c")
  if getrandbits(1):self.mi.append("t_c")
 def is_finished(self):return tm()-self.m_time[0]>self.m_time[1]
 def r_r(self,place):
  of=place*61
  fr(of+4,25,41,13,c[3])
  dt(str(int(self.m_time[1]-(tm()-self.m_time[0]))//60)+":"+str(int(self.m_time[1]-(tm()-self.m_time[0]))%60),of+5,22,c[12],c[3],True)
  fr(of+2,26,2,14,c[0])
  fr(of+4,38,41,2,c[0])
  fr(of+44,37,2,2,c[0])
  if "sa_cuite" in self.mi:ds('sa_cuite',of+26,8,1)
  if "o_c" in self.mi:ds('o_c',of+43,16,1)
 def f_r(self,place):
  of=place*61
  for k in (3,30):fr(of+k,3,30,34,c[3])
  #lines v
  fr(of+3,2,57,2,c[0])
  fr(of+3,36,57,2,c[0])
  fr(of+2,3,2,34,c[0])
...
class P:
 def __init__(self,g,x,y):
  self.x,self.y,self.g,self.di,self.h,assiette=x,y,g,"down",None,A()
  assiette.r(0,0,2)
 def dp(self):
...
class G:
 def __init__(self,mapid):
  self.mapid,self.map=mapid,[]
  for i in ma[mapid]:self.map.append(i[:])
  self.player,self.ms,self.old_missions,self.pmi,self.g_timer,self.g_money=P(self,1,1),[],[],(tm(),1),(tm(),180),0
 def gover(self):
  global hg
  ...
  dt("OK pour aller au menu",55,190,c[0],c[4],True)
 def dms(self):
  if tm()-self.pmi[0]>self.pmi[1]:
   self.ms.append(M())
   self.pmi=(tm(),randint(15,20))
  ...
  for i in range(5):
   for k in range(8):
    ...
    if type(e)==tuple and len(e)==4:
     if tm()>e[2]+e[3]:
      if e[1]=="s":self.map[i][k]=(e[0],"s_cuit")
      if e[1]=="sa":self.map[i][k]=(e[0],"sa_cuite")
      if e[1]=="o":self.map[i][k]=(e[0],"o_c")
      if e[1]=="t":self.map[i][k]=(e[0],"t_c")
 ...
 def gui_f_r(self):
  ...
  fr(48,208,2,14,c[2])
  fr(47,207,2,2,c[2])
  dt("0",2,207,c[12],c[3],True)
  ds("money_icon",38,209)
  fr(269,208,51,14,c[9])
  fr(267,208,2,14,c[8])
  fr(268,207,2,2,c[8])
  dt("0:00",270,207,c[12],c[9],True)
  ds("time_icon",310,209)
 def gui_r_r(self):
  fr(1,208,35,15,c[3])
  dt(str(self.g_money),2,207,c[12],c[3],True)
  fr(0,206,48,2,c[2])
  se=str(int(self.g_timer[1]-(tm()-self.g_timer[0]))%60)
  if len(se)==1:se="0"+se
  dt(str(int(self.g_timer[1]-(tm()-self.g_timer[0]))//60)+":"+se,270,207,c[12],c[9],True)
  fr(269,206,51,2,c[8])
 ...
 def sk(self):
  if kd(KEY_OK) or kd(KEY_HOME) or kd(KEY_POWER):
   self.player.dpa()
   sleep(0.2)
  elif kd(KEY_UP):
   self.player.di="up"
 ...
 def de(self,el,x,y):
  ...
  if e==0:fr(X,Y+40,40,40,c[14])
  if e==1:
   fr(X,Y+40,40,4,c[3])
   fr(X,Y+40+4,40,2,c[1])
   fr(X,Y+40+6,40,8,c[2])
   fr(X,Y+40+14,40,12,c[15])
   fr(X,Y+40+26,40,14,c[14])
  if e==2:fr(X,Y+40,40,40,c[4])
  if e==3:
   fr(X,Y+40,40,40,c[4])
   ds("poele",X,Y+40,2)
  if e==4:
  ...
  if e==5:
   fr(X,Y+40,40,40,c[4])
   ds("assiette",X,Y+42,2)
   ds("assiette",X,Y+38,2)
   ds("assiette",X,Y+34,2)
  if e==6:
   fr(X,Y+40,40,40,c[4])
   ds("c",X,Y+40,2)
   ds("o",X,Y+40,2)
  ...
  if e==11:
   fr(X,Y+40,40,40,c[4])
   ds("poubelle",X,Y+40,2)
  if e==12:
   fr(X,Y+40,40,40,c[4])
   ds("c",X,Y+40,2)
   ds("t",X,Y+40,2)
  if e==13:ds("tapis",X,Y+40,2)
  if type(el)==tuple:ds(el[1],X,Y+40,2)
 ...
 def pi(self,elx,ely,h):
  ...
  if type(h)==I:
   result=None
   e=(old_el,h.i_nom)
   if old_el==11:return None
   elif old_el==2:pass
   elif old_el==3 and h.i_nom=='s':e=(e[0],e[1],tm(),8)
   elif old_el==4 and(h.i_nom=='o' or h.i_nom=='sa' or h.i_nom=='t'):e=(e[0],e[1],tm(),5)
   elif type(old_el)==tuple and type(old_el[1])==A:
    n=h.i_nom
    if n in ("sa_cuite","t_c","o_c","p","s_cuit"):
...
class Gui:
 ...
 def loop(self):
  while 1:
   if kd(KEY_OK) or kd(KEY_HOME):break
...
while 1:
 if tm()-g.g_timer[0]>g.g_timer[1]:
  g.gover()
  while not (kd(KEY_OK) or kd(KEY_HOME) or kd(KEY_POWER)):pass
  gui=Gui()
  sleep(1)
  gui.loop()
  g=G(0)
