...
def ds(sprite_name,x,y,mu=1):
	...
	for i in range(h):
		...
		curx=0;cury+=1
def rhg():
	try:file=open(_P,'r');best=file.readline();file.close();return int(best)
	except:return 0
def shg(score):
	try:file=open(_P,'w');file.truncate(0);file.write(str(score));file.close()
	except:pass
class I:
...
class A:
	...
	def r(self,x,y,mu):
		...
		if _F in self.ai:ds(_F,x,y-2*mu,mu)
...
class P:
	def __init__(self,g,x,y):self.x=x;self.y=y;self.g=g;self.di=_J;self.h=_G;assiette=A();assiette.r(0,0,2)
	def dp(self):
	...
	def mv(self,x_mod,y_mod):
		...
		self.dp()
	def dpa(self):
		dir,e,elx,ely=self.di,_G,_G,_G
		...
		if self.h!=_G:self.h=self.g.pi(elx,ely,self.h);self.dp()
		else:self.h=self.g.ri(elx,ely);self.dp()
class G:
	def __init__(self,mapid):
		self.mapid=mapid;self.map=[]
		for i in ma[mapid]:self.map.append(i[:])
		self.player=P(self,1,1);self.ms=[];self.old_missions=[];self.pmi=tm(),1;self.g_timer=tm(),180;self.g_money=0
	def gover(self):
		...
		for i in range(5):
			for k in range(8):
				...
				if type(e)==tuple and len(e)==4:
					if tm()>e[2]+e[3]:
						if e[1]=='s':self.map[i][k]=e[0],_F
						if e[1]==_I:self.map[i][k]=e[0],_D
	...
	def gui_f_r(self):fr(0,208,48,14,c[3]);fr(0,206,48,2,c[2]);fr(48,208,2,14,c[2]);fr(47,207,2,2,c[2]);dt('0',2,209,c[12],c[3],_A);ds(_K,38,209);fr(272,208,48,14,c[9]);fr(272,206,48,2,c[8]);fr(270,208,2,14,c[8]);fr(271,207,2,2,c[8]);dt('00:00',273,209,c[12],c[9],_A);ds(_O,310,209)
	def gui_r_r(self):
		fr(1,209,30,12,c[3]);dt(str(self.g_money),2,209,c[12],c[3],_A);seconds=str(int(self.g_timer[1]-(tm()-self.g_timer[0]))%60)
	...
	def de(self,el,x,y):
		...
		elif e==7:fr(x*40,y*40+40,40,40,c[4]);ds('c',x*40,y*40+40,2);ds(_I,x*40,y*40+40,2)
	...
	def pi(self,elx,ely,h):
		...
		if type(h)==I:
			result=_G;e=old_el,h.i_nom
			if old_el==11:return
			elif old_el==2:0
			elif old_el==3 and h.i_nom=='s':e=e[0],e[1],tm(),8
			elif old_el==4 and(h.i_nom=='o'or h.i_nom==_I or h.i_nom=='t'):e=e[0],e[1],tm(),5
			elif type(old_el)==tuple and type(old_el[1])==A:
				n=h.i_nom
				if n==_D or n==_E or n==_B or n==_C or n==_F:
			...
			if old_el==11:return
				...
				for i in self.ms:
					if set(i.mi)==set(h.ai):self.g_money+=10;self.ms.remove(i);return
				return h
			else:return h
	def ri(self,elx,ely):
		e=self.map[ely][elx]
...
class Gui:
	...
	def redraw_text(self):dt('A la carte',200,10,c[4],c[15]);dt('Jouer (OK)',200,40,c[3],c[15]);dt('Par Ayabusa',210,205,c[4],c[15])
	def r(self):fr(0,0,320,222,c[15]);ds('ga',0,0,4);self.redraw_text()
	def loop(self):
		while _A:
			if kd(ion.KEY_OK)or kd(ion.KEY_HOME):break
hg=rhg()
gui=Gui()
gui.loop()
g=G(0)
